...
void Update_bounds(double data[], double centroids[], double* c_to_c[], double* centroids_ss[], double* l_elkan[], double u_elkan[], double l_hamerly[], int labels[], double div[], double near_[], int n, int k, int d, long long &feature_cnt) {
    //For all x in X
    for (int i = 0; i < n; i++) {
    ...
    for (int i = 0; i < k; i++) {
        ...
        for (int j = 0; j < k; j++) {
           ...
           if (c_to_c[i][j] < smallest) {
                smallest = c_to_c[i][j];
                near_[i] = 0.5 * smallest;
                near_id = j;
                //near_[j] = 0.5 * smallest;
            }
        }
        //std::cout << i << " is near_ " <<  near_id << "\n";

    }

    //END: Updated l_elkan, u_elkan, l_hamerly, near_, c_to_c
};
